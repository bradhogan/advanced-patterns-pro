...
/**
 * Register custom block attributes for ID and rel
 */
function appro_register_block_id_rel_attributes() {
	$block_types = WP_Block_Type_Registry::get_instance()->get_all_registered();
	
	foreach ( $block_types as $block_type ) {
		// Skip if block already has these attributes registered
		if ( isset( $block_type->attributes['blockId'] ) || isset( $block_type->attributes['blockRel'] ) ) {
			continue;
		}
...
function appro_add_block_id_rel_attributes( $block_content, $block ) {
	...
	if ( $processor->next_tag() ) {
		// Add ID attribute if set
		if ( ! empty( $block_id ) ) {
			$processor->set_attribute( 'id', esc_attr( $block_id ) );
		}
		
		// Add rel attribute if set
		if ( ! empty( $block_rel ) ) {
